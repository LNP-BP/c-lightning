						  ...
						  struct bitcoin_signature *commit_sig)
{
	...
	for (i = 0; i < tal_count(htlc_sigs); i++) {
		...
		status_debug("Creating HTLC signature %s for tx %s wscript %s key %s",
			     ...
			     tal_hex(tmpctx, txs[i+1]->output_witscripts[0]),
			     type_to_string(tmpctx, struct pubkey,
					    &local_htlckey));
		assert(check_tx_sig(txs[1+i], 0, NULL, txs[i+1]->output_witscripts[0],
				    &local_htlckey,
				    &sig));
	}
...
static void handle_peer_commit_sig(struct peer *peer, const u8 *msg)
{
	...
	for (i = 0; i < tal_count(htlc_sigs); i++) {
		...
		if (!check_tx_sig(txs[1+i], 0, NULL, txs[1+i]->output_witscripts[0],
				  &remote_htlckey, &sig))
			peer_failed(peer->pps,
				    &peer->channel_id,
				    ...
				    tal_hex(msg, txs[1+i]->output_witscripts[0]),
				    type_to_string(msg, struct pubkey,
						   &remote_htlckey));
	}
